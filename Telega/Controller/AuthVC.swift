...
class AuthVC: UIViewController {
	...
	@IBAction func createAccntTogglePressed(_ sender: Any) {
		...
		let newToggleTitle = creatingAccnt ? "I already have an account"
	...
	private func getCredentials(
		...
		guard let email = emailTxtFld.text, emailTxtFld.text != ""
		else { return nil }
		if !isValidEmail(testStr: email) {
			emailTxtFld.shake()
			return nil
		}
		guard let password = passwordTxtFld.text, passwordTxtFld.text != ""
		else { return nil }
